...
package it.uniroma3.agiw1.bing;
...
import it.uniroma3.agiw1.main.SyncObj;
...
public class QueryExtractorSlave implements Runnable {
	...
	private void extract() {
		...
		if(EngineBuilder.flags.contains(Flags.Mongo_build_DOC))
			this.mongoConnector.bulkAdd(customJSON.getJSONArray("doc_collection"),EngineBuilder.mongoQueryCollection);
		if(EngineBuilder.flags.contains(Flags.Mongo_build_HTML))
			this.mongoConnector.bulkAdd(customJSON.getJSONArray("html_collection"), EngineBuilder.mongoHTMLCollection);
		if(EngineBuilder.flags.contains(Flags.FS_HTML))
			bulkSaveHTML_FILES(query,customJSON.getJSONArray("html_collection"));
		if(EngineBuilder.flags.contains(Flags.Elastic_build_index))
			this.elasticConnector.bulkPut(customJSON.getJSONArray("doc_collection"), EngineBuilder.elasticIndexName, EngineBuilder.elasticTypeName);
	}
	...
	private void saveHTML_FILE(JSONObject customJSON) {
		...
		String path = EngineBuilder.htmlFolder+"/"+folder_name;
